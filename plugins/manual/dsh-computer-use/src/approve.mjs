// 敏感动作（click/type/key/scroll/menu/app）的人工审批：走 userQuestions.ask ——
// 只有 live runtime root 能问人，被 subagent 拥有的 agent 会被 DELEGATED_CALLER 拒绝（fail-closed）。
// 机制事实见 knowledge/foundations/tool-approval-interception-and-secrets.md；
// 人不可达、拒绝或异常时一律 fail-closed。
// 默认关（approveActions=false，2026-08-28 用户决议：整机控制风险面高，但与 P0 一致先默认关）。

const OPTIONS = [
  { label: '允许一次', description: '仅放行这一次整机动作。' },
...
export async function requestActionApproval(ctx, { agent, tool, detail }) {
  ...
  try {
    const answer = await questions.ask({
      ...
      questions: [
        {
          id: 'computer-action-approve',
          header: '整机动作审批',
          question: '是否允许执行整机动作「' + tool + '」？',
          detail,
          options: OPTIONS,
        },
      ],
