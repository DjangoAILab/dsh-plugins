  ...
  const work = (async () => {
    ...
    try {
      outcome = await registerApp({
        // SDK ≥1.61.x 回调名是 onQRCodeReady（对象 { url, expireIn }），不是 onQrCode。
        onQRCodeReady: (info) => emitQr(String(info?.url ?? '')),
        // SDK ≥1.61.x 的 addons 形为 { preset, scopes, events, callbacks }，不再有 resources。
        // 最小权限集（对应 feishu-bot.mjs 长连接收发能力）：
        //   preset: false —— 丢弃默认模板，最小基座（仅机器人能力，无业务权限）；
        //   scopes.tenant —— 应用身份权限（发 + 读都要，否则收不到消息事件）：
        //     im:message:send_as_bot（发消息/回执/卡片） + 5 个读权限（接收单聊/群聊消息）。
        //   events.items.tenant —— 应用身份事件：im.message.receive_v1（接收私聊/群聊消息）；
        //   callbacks.items —— 卡片回调：card.action.trigger（审批/停止按钮）。
        addons: {
          preset: false,
          scopes: { tenant: [
      ...
      // SDK registerApp 失败时 reject 的是普通对象 { code, description }，不是 Error。
      const code = String(error?.code ?? '')
