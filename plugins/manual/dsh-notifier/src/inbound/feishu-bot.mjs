...
export function createFeishuInbound({ config, bus, fallbackTargets = [], logger = null, sdkLoader, actions = null, identity = null } = {}) {
  ...
  function handleMessage(data) {
    try {
      // [DEBUG-tmp] 临时排障日志：写文件（绕过宿主日志拦截）
      try { appendFileSync('/tmp/feishu-debug.log', '[receive] ' + JSON.stringify(data).slice(0, 2000) + '\n') } catch { /* noop */ }
      const message = data?.message ?? {}
      const openId = String(data?.sender?.sender_id?.open_id ?? '')
      const messageId = String(message.message_id ?? '')
      try { appendFileSync('/tmp/feishu-debug.log', '[parsed] openId=' + openId + ' messageId=' + messageId + ' msgType=' + String(message.message_type ?? '') + ' chatType=' + String(message.chat_type ?? '') + '\n') } catch { /* noop */ }
